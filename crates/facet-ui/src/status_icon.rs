//! The tray icon, drawn for whatever state the app is in.
//!
//! **Shared, because there is one of it.** This is the pixels and the geometry, and neither is a platform
//! question: a composition root takes [`render`] and wraps the result in whatever its own tray API wants,
//! which is an `Icon` on macOS and Windows and an ARGB pixmap on Linux. Writing a second copy per platform
//! is how three trays start disagreeing about what paused looks like.
//!
...
//! is one more glyph here and eight files the other way. `facet-mac/examples/draw-status-icons.rs` writes
//! the four images out, so they can still be looked at as files.
//!
//! **A non-square icon is not something every platform will take**, and that is the one thing about this
//! shape a caller has to know:
//!
//! - **macOS sizes a status item by aspect, not by fitting a square.** tray-icon asks for an 18 point tall
//!   image and computes the width from the ratio, so a 70 by 32 bitmap becomes a 39 by 18 point item and
//!   the status item widens to hold it. Nothing is squashed and nothing shrinks.
//! - **Windows cannot do this.** A tray icon there is an `HICON` drawn into the shell's own square slot, so
//!   a 2:1 image is squashed rather than given room. Two same-size glyphs side by side is a macOS and Linux
//!   shape; Windows needs both inside one square, which means each is about half the size, or a badge in a
//!   corner. That is the Windows adapter's decision and `docs/port-findings.md` records it, so it is not
//!   met as a squashed icon.
//! - **Linux takes its own width and height.** ksni's `icon_pixmap` carries both and the StatusNotifierItem
//!   specification permits a non-square icon. Whether the applet on the other end honours it is a question
//!   about that applet, and the answer belongs in `docs/port-findings.md`.
//!
